...
,__A:function(cR,cS,name,cT,cU,cV){var cW=this.$$method[cT][name];cS[cW]=new Function("value",cU.join(""));{}
...
(function(){var h="[Class ",g="]",f="extend",e="qx.Class",d=".",c="static",b="Array";qx.Bootstrap.define(e,{statics:{__M:qx.core.Environment.get("module.property")?qx.core.Property:null,define:function(name,j){if(!j){var j={};}
...
;{}
;var m=this.__P(name,j.type,j.extend,j.statics,j.construct,j.destruct,j.include);if(j.extend){if(j.properties){this.__R(m,j.properties,true);}
...
;}
;{}
;if(j.defer){j.defer.self=m;j.defer(m,m.prototype,{add:function(name,o){var p={};p[name]=o;qx.Class.__R(m,p,true);}
});}
;return m;}
...
else {var bb={};if(T){if(!V){V=this.__Y();}
...
;}
;}
,__S:null,__T:function(bm,bn,bo,bp,bq){var br=bm.prototype;var bt,bs;qx.Bootstrap.setDisplayNames(bn,bm.classname+".prototype");for(var i=0,a=qx.Bootstrap.getKeys(bn),l=a.length;i<l;i++){bt=a[i];bs=bn[bt];{}
...
;}
,__W:function(bB){var name=bB.classname;var bC=this.__bc(bB,name,bB.$$classtype);for(var i=0,a=qx.Bootstrap.getKeys(bB),l=a.length;i<l;i++){bD=a[i];bC[bD]=bB[bD];}
...
(function(){var m="-",k="",j="qx.core.ObjectRegistry",h="Disposed ",g="-0",f=" objects",e="Could not dispose object ",d=": ",c="$$hash";qx.Class.define(j,{statics:{inShutDown:false,__j:{},__bH:0,__bI:[],__bJ:k,__bK:{},register:function(n){var q=this.__j;if(!q){return;}
...
;var t=this.__j;if(t&&t[s]){delete t[s];this.__bI.push(s);}
;try{delete r.$$hash;}
...
(function(){var k='"',j="Unsupported type!",h="change",g="qx.data.marshal.Json",f="set",e="_applyEventPropagation",d="",c="Array",b="_validate",a="qx.data.model.";qx.Class.define(g,{extend:qx.core.Object,implement:[qx.data.marshal.IMarshaler],construct:function(l){qx.core.Object.call(this);this.__ds=l;}
...
},members:{__ds:null,__dt:function(o){return qx.Bootstrap.getKeys(o).sort().join(k);}
...
;}
;return;}
;var s=this.__dt(p);for(var t in p){this.toClass(p[t],q);}
;if(qx.Class.isDefined(a+s)){return;}
;if(this.__ds&&this.__ds.getModelClass&&this.__ds.getModelClass(s)!=null){return;}
;var z={};var y={__du:this.__du};for(var t in p){if(this.__ds&&this.__ds.getPropertyMapping){t=this.__ds.getPropertyMapping(t,s);}
;t=t.replace(/-|\.|\s+/g,d);{}
;z[t]={};z[t].nullable=true;z[t].event=h+qx.lang.String.firstUp(t);if(q){z[t].apply=e;}
;if(this.__ds&&this.__ds.getValidationRule){var v=this.__ds.getValidationRule(s,t);if(v){z[t].validate=b+t;y[b+t]=v;}
;}
;}
;if(this.__ds&&this.__ds.getModelSuperClass){var x=this.__ds.getModelSuperClass(s)||qx.core.Object;}
else {var x=qx.core.Object;}
;var u=[];if(this.__ds&&this.__ds.getModelMixins){var w=this.__ds.getModelMixins(s);if(!qx.lang.Type.isArray(w)){if(w!=null){u=[w];}
;}
else {u=w;}
;}
;if(q){u.push(qx.data.marshal.MEventBubbling);}
;var r={extend:x,include:u,properties:z,members:y,destruct:this.__dv};qx.Class.define(a+s,r);}
,__dv:function(){var A=qx.util.PropertyUtil.getAllProperties(this.constructor);for(var B in A){this.__du(this.get(A[B].name));}
;}
,__du:function(C){if(!(C instanceof qx.core.Object)){return;}
;if(C.isDisposed()){return;}
;C.dispose();}
,__dw:function(D){var E;if(this.__ds&&this.__ds.getModelClass){E=this.__ds.getModelClass(D);}
...
;return N;}
else if(L){var I=this.__dt(G);var O=this.__dw(I);for(var M in G){var J=M;if(this.__ds&&this.__ds.getPropertyMapping){J=this.__ds.getPropertyMapping(M,I);}
...
,_registerEventChaining:function(m,n,name){if((m instanceof qx.core.Object)&&qx.Class.hasMixin(m.constructor,qx.data.marshal.MEventBubbling)){var o=qx.lang.Function.bind(this.__dx,this,name);var q=m.addListener(f,o,this);var p=m.getUserData(b+this.$$hash);if(p==null){p=[];m.setUserData(b+this.$$hash,p);}
;p.push(q);}
...
;}
,__dx:function(name,e){var y=e.getData();var u=y.value;var s=y.old;if(qx.Class.hasInterface(e.getTarget().constructor,qx.data.IListData)){if(y.name.indexOf){var x=y.name.indexOf(d)!=-1?y.name.indexOf(d):y.name.length;var v=y.name.indexOf(a)!=-1?y.name.indexOf(a):y.name.length;if(v==0){var t=name+y.name;}
...
else if(arguments.length>1){this.__dy=[];for(var i=0;i<arguments.length;i++){this.__dy.push(arguments[i]);}
;}
else if(typeof q==m){this.__dy=new Array(q);}
else if(q instanceof Array){this.__dy=qx.lang.Array.clone(q);}
else {this.__dy=[];this.dispose();throw new Error(n);}
;;;for(var i=0;i<this.__dy.length;i++){this._applyEventPropagation(this.__dy[i],null,i);}
;this.__dz();{}
;}
,properties:{autoDisposeItems:{check:p,init:false}},events:{"change":h,"changeLength":h},members:{__dy:null,concat:function(r){if(r){var s=this.__dy.concat(r);}
else {var s=this.__dy.concat();}
;return new qx.data.Array(s);}
,join:function(t){return this.__dy.join(t);}
,pop:function(){var u=this.__dy.pop();this.__dz();this._registerEventChaining(null,u,this.length-1);this.fireDataEvent(a,{value:[],name:this.length+d,old:[u],item:this});this.fireDataEvent(b,{start:this.length-1,end:this.length-1,type:c,items:[u]},null);return u;}
,push:function(v){for(var i=0;i<arguments.length;i++){this.__dy.push(arguments[i]);this.__dz();this._registerEventChaining(arguments[i],null,this.length-1);this.fireDataEvent(a,{value:[arguments[i]],name:(this.length-1)+d,old:[],item:this});this.fireDataEvent(b,{start:this.length-1,end:this.length-1,type:e,items:[arguments[i]]},null);}
;return this.length;}
,reverse:function(){if(this.length==0){return;}
;var w=this.__dy.concat();this.__dy.reverse();this.fireDataEvent(b,{start:0,end:this.length-1,type:g,items:null},null);this.fireDataEvent(a,{value:this.__dy,name:f+(this.__dy.length-1),old:w,item:this});}
,shift:function(){if(this.length==0){return;}
;var x=this.__dy.shift();this.__dz();this._registerEventChaining(null,x,this.length-1);this.fireDataEvent(a,{value:[],name:j,old:[x],item:this});this.fireDataEvent(b,{start:0,end:this.length-1,type:c,items:[x]},null);return x;}
,slice:function(y,z){return new qx.data.Array(this.__dy.slice(y,z));}
,splice:function(A,B,C){var K=this.__dy.length;var G=this.__dy.splice.apply(this.__dy,arguments);if(this.__dy.length!=K){this.__dz();}
;var I=B>0;var E=arguments.length>2;var F=null;if(I||E){if(this.__dy.length>K){var J=e;F=qx.lang.Array.fromArguments(arguments,2);}
else if(this.__dy.length<K){var J=c;F=G;}
else {var J=g;}
...
;var M=this.__dy.concat();this.__dy.sort.apply(this.__dy,arguments);if(qx.lang.Array.equals(this.__dy,M)===true){return;}
;this.fireDataEvent(b,{start:0,end:this.length-1,type:g,items:null},null);this.fireDataEvent(a,{value:this.__dy,name:f+(this.length-1),old:M,item:this});}
,unshift:function(N){for(var i=arguments.length-1;i>=0;i--){this.__dy.unshift(arguments[i]);this.__dz();this._registerEventChaining(arguments[i],null,0);this.fireDataEvent(a,{value:[this.__dy[0]],name:j,old:[this.__dy[1]],item:this});this.fireDataEvent(b,{start:0,end:this.length-1,type:e,items:[arguments[i]]},null);}
;return this.length;}
,toArray:function(){return this.__dy;}
,getItem:function(O){return this.__dy[O];}
,setItem:function(P,Q){var R=this.__dy[P];if(R===Q){return;}
;this.__dy[P]=Q;this._registerEventChaining(Q,R,P);if(this.length!=this.__dy.length){this.__dz();}
;this.fireDataEvent(a,{value:[Q],name:P+d,old:[R],item:this});this.fireDataEvent(b,{start:P,end:P,type:e,items:[Q]},null);}
,getLength:function(){return this.length;}
,indexOf:function(S){return this.__dy.indexOf(S);}
,toString:function(){if(this.__dy!=null){return this.__dy.toString();}
;return d;}
,contains:function(T){return this.__dy.indexOf(T)!==-1;}
...
,removeAll:function(){for(var i=0;i<this.__dy.length;i++){this._registerEventChaining(null,this.__dy[i],i);}
;if(this.getLength()==0){return;}
;var bh=this.getLength();var bg=this.__dy.concat();this.__dy.length=0;this.__dz();this.fireDataEvent(a,{value:[],name:f+(bh-1),old:bg,item:this});this.fireDataEvent(b,{start:0,end:bh-1,type:c,items:bg},null);return bg;}
,append:function(bi){if(bi instanceof qx.data.Array){bi=bi.toArray();}
;{}
;Array.prototype.push.apply(this.__dy,bi);for(var i=0;i<bi.length;i++){this._registerEventChaining(bi[i],null,this.__dy.length+i);}
;var bj=this.length;this.__dz();var name=bj==(this.length-1)?bj:bj+k+(this.length-1);this.fireDataEvent(a,{value:bi,name:name+d,old:[],item:this});this.fireDataEvent(b,{start:bj,end:this.length-1,type:e,items:bi},null);}
...
,forEach:function(bq,br){for(var i=0;i<this.__dy.length;i++){bq.call(br,this.__dy[i],i,this);}
;}
,__dz:function(){var bs=this.length;this.length=this.__dy.length;this.fireDataEvent(l,this.length,bs);}
},destruct:function(){for(var i=0;i<this.__dy.length;i++){var bt=this.__dy[i];this._applyEventPropagation(null,bt,i);if(this.isAutoDisposeItems()&&bt&&bt instanceof qx.core.Object){bt.dispose();}
;}
;this.__dy=null;}
});}
)();
